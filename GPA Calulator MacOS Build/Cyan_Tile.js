...
Profile_Data.Major_Program = (document.getElementById("Program_Label").innerHTML);
  ...
  var Path_To_Profile_Data = localStorage.getItem('Profile_Path');
  fs.writeFile(Path_To_Profile_Data+'Main_Profile.json',Profile_Data_Output_Stream,Completed);
...
let Desktop_Directory = Home_Directory + "/Desktop/GPA_Grapher.png";
...
Main_Window.webContents.capturePage().then(Grapher_Image =>
    {
      //writing  image to the disk
          fs.writeFile(Desktop_Directory, Grapher_Image.toPNG(), (err) => {
          if (err) throw err
...
function Copy_Image(Image_File_Path){
  ...
  var Path_Name = localStorage.getItem('Path_Prefix');
  var Path_To_Profile_Data = localStorage.getItem('Profile_Path');
  var Profile_Copy_Path = Path_To_Profile_Data + "Profile_Image.png";

// destination will be created or overwritten by default.
fs.copyFile(Image_File_Path, Profile_Copy_Path, (err) => {
